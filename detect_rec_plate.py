...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='T_T_result', help='source')  # 结果保存的文件夹
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    for pic_ in file_list:  # for循环依次读取列表中存放的文件地址
        ...
        if file_type in ".jpg .png":  # 对文件夹中的.jpg和.png格式的图片进行检测
            ...
            ori_img, _ = draw_result(img, result_list)  # 原图上绘制检测框
            img_name = os.path.basename(pic_)  # 获取图片名称
    ...
    print(f"检测共计耗时： {time.time() - time_begin} s")
